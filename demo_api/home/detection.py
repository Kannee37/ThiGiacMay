...
def load_model(model_path):
    model_path = 'D:/mine/HocTap/ThiGiacMay/best.pt'
...
def cut_single_image(model, img_path):
    ...
    # cv2.imshow("Detection Result", result_img)  # Hiển thị ảnh với bounding box
    # cv2.waitKey(0)
    ...
    return cropped_plate, result_img

# Hàm cắt ảnh đơn (một ảnh)
def cut_mage_folder(model, img_path, output_dir):
    # Đọc ảnh từ đường dẫn
    img = cv2.imread(img_path)
    if img is None:
        print(f"Không thể đọc ảnh từ đường dẫn: {img_path}")
...
# Hàm vẽ bounding box lên ảnh mà không cắt ra
def draw_bounding_boxes_on_image(model, img_path):
    # Đọc ảnh từ đường dẫn
    ...
    # Sử dụng phương thức plot() để vẽ bounding box và nhãn lên ảnh gốc
    result_img = results[0].plot()  # Tự động vẽ bounding box và nhãn lên ảnh

    # Hiển thị kết quả nhận diện bằng OpenCV
    # cv2.imshow("Detection Result", result_img)  # Hiển thị ảnh với bounding box
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
...
def detect_plate(model, img_path, output_dir = None, input_dir=None):
    ...
    return cut_single_image(model, img_path)
